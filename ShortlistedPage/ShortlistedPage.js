async function fetchApplicantsData() {
  try {
    const response = await fetch('../HirePage/applicants.json');
...
async function initializeApp() {
    ...
    filteredApplicants.forEach(applicant => {
      if (applicant.shortlist) {
        ...
        shortlistBtn.addEventListener('click', () => {
          applicant.shortlist = false; // Update shortlist value in memory
  ...
  function applyFilters() {
    ...
    filteredApplicants = applicantsData.applicants.filter(applicant => applicant.shortlist);
